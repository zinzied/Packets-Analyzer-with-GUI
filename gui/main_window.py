...
class MainWindow(QMainWindow):
    ...
    def populate_interface_selector(self):
        ...
        try:
            ...
            for adapter_name, addresses in adapters.items():
                ...
                is_up = adapter_stats.get(adapter_name, None)
                status = "Up" if (is_up and is_up.isup) else "Down"

                # Add to combo box with descriptive text
                self.interface_selector.addItem(f"{adapter_name} ({ip_address}) - {status}", adapter_name)
    ...
    def start_capture(self):
        ...
        # Get selected interface (stored in the userData of the combo box item)
        selected_index = self.interface_selector.currentIndex()
        if selected_index < 0:
            self.show_error("No interface selected")
            return

        # Get the actual interface name (not the display text)
        interface_data = self.interface_selector.itemData(selected_index)
        interface = interface_data if interface_data else self.interface_selector.currentText()
        ...
        # Update status
        self.update_status(f"Started capturing on interface: {interface}")
    ...
    def save_capture_file(self):
        ...
        packet_count = self.packet_list.count()
        if packet_count == 0:
