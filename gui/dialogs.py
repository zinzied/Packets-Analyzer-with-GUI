...
class ScanDialog(QDialog):
    ...
    def populate_adapter_combo(self):
        ...
        try:
            ...
            for adapter_name, addresses in adapters.items():
                ...
                self.adapter_combo.addItem(f"{adapter_name} ({ip_address}) - {status}", adapter_name)
    ...
    def start_scan(self):
        ...
        # Get selected adapter (stored in the userData of the combo box item)
        selected_index = self.adapter_combo.currentIndex()
        if selected_index < 0:
            QtWidgets.QMessageBox.warning(self, "Error", "No network adapter selected")
            return

        adapter_name = self.adapter_combo.itemData(selected_index)

        # Get IP range
        ip_range = self.ip_range_input.text().strip()
        if not ip_range:
            # Try to auto-fill IP range based on selected adapter
            try:
                adapters = psutil.net_if_addrs()
                if adapter_name in adapters:
                    for addr in adapters[adapter_name]:
                        if addr.family == socket.AF_INET:  # IPv4
                            # Extract first 3 octets of IP and add /24
                ...
                logging.error(f"Error auto-filling IP range: {str(e)}")

            # If still no IP range, show error
            if not ip_range:
